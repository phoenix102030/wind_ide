...
def load_rows(path: Path) -> dict[str, dict[str, np.ndarray]]:
    ...
    for row in rows:
        ...
        metrics[model]["rmse"].append(float(row["rmse"]))
        metrics[model]["mae"].append(float(row["mae"]))
        metrics[model]["crps"].append(float(row["crps"]))
        metrics[model]["mape"].append(float(row["mape_percent"]))
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot hourly speed metrics from CSV.")
